...
from utilities.nutrient_list import *
...
def fuz_check_for_label(text, fuzdict):
...
def get_label_from_string(string):
    ...
    value_arr = re.findall(
        "{0}g|{0}%|{0}J|{0}kJ|{0}mg|{0}kcal".format(digit_pattern), string
    )
    # print(value_arr)
    if len(value_arr):
...
# Separate the value and its label from the string
def get_fuz_label_from_string(string, fuzdict):
    ...
    digit_pattern = "[-+]?\d*[\.\,']?\d+"
    value_arr = re.findall(
        "{0}g|{0}%|{0}J|{0}kJ|{0}mg|{0}kcal|{0}".format(digit_pattern), string
    )
    if len(value_arr):
        label_value = value_arr[0]
    else:
...
def main():
    ...
    ap.add_argument(
        "-s", "--string", required=True, help="Enter the string to be cleaned"
    )
