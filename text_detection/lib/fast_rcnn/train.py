...
from text_detection.lib.fast_rcnn.config import cfg
from text_detection.lib.roi_data_layer import roidb as rdl_roidb
from text_detection.lib.roi_data_layer.layer import RoIDataLayer
from text_detection.lib.utils.timer import Timer
...
class SolverWrapper(object):
    def __init__(
        self, sess, network, imdb, roidb, output_dir, logdir, pretrained_model=None
    ):
    ...
    def snapshot(self, sess, iter):
        ...
        if (
            ...
            sess.run(
                weights.assign(orig_0 * np.tile(self.bbox_stds, (weights_shape[0], 1)))
            )
        ...
        filename = (
            cfg.TRAIN.SNAPSHOT_PREFIX + infix + "_iter_{:d}".format(iter + 1) + ".ckpt"
        )
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        if self.pretrained_model is not None and not restore:
            try:
                print(
                    ("Loading pretrained model " "weights from {:s}").format(
                ...
                raise Exception(
                    "Check your pretrained model {:s}".format(self.pretrained_model)
                )
        ...
        for iter in range(restore_iter, max_iters):
            ...
            if (iter) % (cfg.TRAIN.DISPLAY) == 0:
                print(
                    "iter: %d / %d, total loss: %.4f, model loss: %.4f,"
