...
from text_detection.lib.fast_rcnn.nms_wrapper import nms

from .text_connect_cfg import Config as TextLineCfg
from .text_proposal_connector import TextProposalConnector
from .text_proposal_connector_oriented import TextProposalConnector as TextOriented
...
class TextDetector:
    def __init__(self):
        ...
        if self.mode == "H":
            ...
            self.text_proposal_connector = TextOriented()
    ...
    def detect(self, text_proposals, scores, size):
        ...
        # 对proposal做nms
        keep_inds = nms(
            np.hstack((text_proposals, scores)), TextLineCfg.TEXT_PROPOSALS_NMS_THRESH
        )
        text_proposals, scores = text_proposals[keep_inds], scores[keep_inds]
    ...
    def filter_boxes(self, boxes):
        ...
        return np.where(
            (widths / heights > TextLineCfg.MIN_RATIO)
            & (scores > TextLineCfg.LINE_MIN_SCORE)
            & (
                widths
                > (TextLineCfg.TEXT_PROPOSALS_WIDTH * TextLineCfg.MIN_NUM_PROPOSALS)
            )
        )[0]
