...
from text_detection.lib.fast_rcnn.bbox_transform import bbox_transform_inv, clip_boxes
...
def proposal_layer(
    ...
    # (1, H, W, A)
    scores = np.reshape(
        np.reshape(rpn_cls_prob_reshape, [1, height, width, _num_anchors, 2])[
            :, :, :, :, 1
        ],
    ...
    A = _num_anchors
    K = shifts.shape[0]
    anchors = _anchors.reshape((1, A, 4)) + shifts.reshape((1, K, 4)).transpose(
        (1, 0, 2)
    )
