...
from text_detection.lib.fast_rcnn.bbox_transform import bbox_transform
from text_detection.lib.fast_rcnn.config import cfg
from text_detection.lib.utils.bbox import bbox_intersections, bbox_overlaps
...
def anchor_target_layer(
    ...
    ).transpose()  # 生成feature-map和真实image上anchor之间的偏移量
    ...
    all_anchors = _anchors.reshape((1, A, 4)) + shifts.reshape((1, K, 4)).transpose(
        (1, 0, 2)
    )  # 相当于复制宽高的维度，然后相加
    ...
    gt_argmax_overlaps = overlaps.argmax(
        axis=0
    )  # G#找到每个位置上9个anchor中与gtbox，overlap最大的那个
    ...
    if not cfg.TRAIN.RPN_CLOBBER_POSITIVES:
        ...
        labels[
            max_overlaps < cfg.TRAIN.RPN_NEGATIVE_OVERLAP
        ] = 0  # 先给背景上标签，小于0.3overlap的
    ...
    if (
        dontcare_areas is not None and dontcare_areas.shape[0] > 0
    ...
    if (
        cfg.TRAIN.PRECLUDE_HARD_SAMPLES
        and gt_ishard is not None
        and gt_ishard.shape[0] > 0
    ):
