...
from text_detection.lib.utils.blob import im_list_to_blob, prep_im_for_blob
...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        blobs["gt_ishard"] = (
            roidb[0]["gt_ishard"][gt_inds]
            if "gt_ishard" in roidb[0]
            else np.zeros(gt_inds.size, dtype=int)
        )
        # blobs['gt_ishard'] = roidb[0]['gt_ishard'][gt_inds]
        blobs["dontcare_areas"] = (
            roidb[0]["dontcare_areas"] * im_scales[0]
            if "dontcare_areas" in roidb[0]
        ...
        for im_i in range(num_images):
            labels, overlaps, im_rois, bbox_targets, bbox_inside_weights = _sample_rois(
                roidb[im_i], fg_rois_per_image, rois_per_image, num_classes
            )
        ...
        if cfg.TRAIN.BBOX_REG:
            ...
            blobs["bbox_outside_weights"] = np.array(bbox_inside_blob > 0).astype(
                np.float32
            )
